...
from .config import DynamicsConfig, SensorConfig, SimulationConstants
from .models import MissionZone, Vector3, vec3
from .models import ZONE_TYPE_EXCLUSION, ZONE_TYPE_OBJECTIVE, ZONE_TYPE_PATROL, ZONE_TYPE_SURVEILLANCE
...
def apply_difficulty_scaling(
    ...
    for name, profile in constants.platform_presets.items():
        from .config import PlatformPresetProfile
...
def generate_mission(spec: MissionSpec) -> GeneratedMission:
    ...
    from .sim import build_default_scenario, ScenarioOptions
